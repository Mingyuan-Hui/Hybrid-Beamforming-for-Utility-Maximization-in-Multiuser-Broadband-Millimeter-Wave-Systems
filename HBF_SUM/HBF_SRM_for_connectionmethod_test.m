function [Rate,rateresp,Fi,Wi,Ti] = HBF_SRM_for_connectionmethod_test(H,h,F_RF,if_fc,Nk, Nt, Nr, Vn, Ns, Ntrf, K, Iter, W_RF,Nrrf,Pt,B,A,W)
...
for i = 1:Nk
    ...
    %% Frf ini
    F_RF = ciw_frf_mo_algorithm_5_connectionmethod(F_RF,v ,T, H2,if_fc,Nt,Ntrf,Nk);
    for i = 1:Nk
...
for ite =1:Iter
    ...
    for m3 = 1:K
            ...
            W_RF(:,:,m3) = ciw_wrf_mo_algorithm(R(:,:,m3,:),S(:,:,m3,:),W_RF(:,:,m3),T((m3-1)*Ns+1:m3*Ns,(m3-1)*Ns+1:m3*Ns,:),1,Nr,Nrrf,Nk);      
    end
    WRF = W_RF(:,:,1);
for k =2 :K 
    WRF=blkdiag(WRF, W_RF(:,:,k));
