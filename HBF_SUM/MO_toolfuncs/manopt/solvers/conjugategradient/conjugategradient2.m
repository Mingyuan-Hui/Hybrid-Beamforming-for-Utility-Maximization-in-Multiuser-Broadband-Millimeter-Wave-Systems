function [x, cost, info, options] = conjugategradient2(problem, x, options)
...
%   linesearch (@linesearch_adaptive2 or @linesearch_hint)
...
%       allow it. The default linesearch_adaptive2 used does exploit the
...
localdefaults.maxiter = 5; %yuanbenshi1000
...
if ~canGetLinesearch(problem)
    localdefaults.linesearch = @linesearch_adaptive2;
else
...
[cost grad storedb] = getCostGrad2(problem, x, storedb);
...
while true
    ...
    [stop reason] = stoppingcriterion2(problem, x, options, info, iter+1);
    ...
    [newcost newgrad storedb] = getCostGrad2(problem, newx, storedb);
